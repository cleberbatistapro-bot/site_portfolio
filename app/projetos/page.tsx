...
function OrganizerCaseStudy() {
  ...
  return <>
    ...
    <section className="project-section project-solution">
      <div className="project-section-copy"><span className="project-section-number">02</span><h3>A solução desenvolvida</h3><p>Desenvolvi uma aplicação desktop que permite escolher a pasta de origem, configurar opções de segurança e executar a organização de forma controlada. O programa lê as extensões, aplica as regras de classificação e direciona cada arquivo para a categoria correspondente.</p></div>
    ...
    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">03</span><h3>Como funciona</h3><p>O fluxo foi desenhado para ser compreendido mesmo por quem não possui conhecimento técnico.</p></div><ol className="project-process"><li><span>01</span><h4>Selecionar</h4><p>O usuário indica a pasta que contém os arquivos.</p></li><li><span>02</span><h4>Configurar</h4><p>Define simulação, backup e destino da organização.</p></li><li><span>03</span><h4>Organizar</h4><p>A aplicação identifica os formatos e aplica as regras.</p></li><li><span>04</span><h4>Conferir</h4><p>O relatório apresenta os resultados da execução.</p></li></ol></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">04</span><h3>Segurança antes da execução</h3></div><div className="project-security"><Icon name="shield" size={34}/><div><p>A aplicação prioriza a previsibilidade. O usuário pode simular a organização e gerar uma cópia de segurança antes de confirmar qualquer movimentação.</p><div className="project-checks"><span><Icon name="check" size={18}/>Prévia antes das alterações</span><span><Icon name="check" size={18}/>Cópia de segurança opcional</span><span><Icon name="check" size={18}/>Arquivos não reconhecidos preservados</span></div></div></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">05</span><h3>Resultado da demonstração</h3><p>Dados observados no teste apresentado, sem estimativas de produtividade ou resultados comerciais.</p></div><div className="project-metrics"><article><strong>21</strong><span>arquivos processados</span><p>Total identificado na pasta de teste.</p></article><article><strong>21</strong><span>arquivos organizados</span><p>Todos direcionados para suas categorias.</p></article><article><strong>0</strong><span>não reconhecidos</span><p>Nenhum formato ficou sem classificação.</p></article></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">06</span><h3>Antes e depois</h3></div><div className="project-comparison"><article className="before"><header><div><strong>Antes</strong><p>Arquivos misturados e sem padrão.</p></div><Icon name="warning"/></header><ul>{before.map((file) => <li key={file}><Icon name="file" size={16}/><span>{file}</span></li>)}</ul></article><article className="after"><header><div><strong>Depois</strong><p>Arquivos separados por categoria.</p></div><Icon name="check"/></header><ul>{after.map(([folder, count]) => <li key={folder}><Icon name="folder" size={16}/><span>{folder}</span><small>{count}</small></li>)}</ul></article></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">07</span><h3>Tecnologias e competências</h3><p>O projeto combina implementação técnica com análise de uma rotina administrativa real.</p></div><div className="project-skills-grid"><article><h4>Tecnologias utilizadas</h4><div>{technologies.map(item => <span key={item}>{item}</span>)}</div></article><article><h4>Competências demonstradas</h4><div>{skills.map(item => <span key={item}>{item}</span>)}</div></article></div></section>
...
function ProcFacilCaseStudy() {
  ...
  return <>
    ...
    <section className="project-section project-solution">
      <div className="project-section-copy"><span className="project-section-number">02</span><h3>A solução desenvolvida</h3><p>Desenvolvi uma aplicação que conduz o usuário por três etapas. Primeiro, ele seleciona duas ou mais planilhas. Depois, escolhe a aba, a coluna-chave e quais campos de cada arquivo devem entrar no resultado. Por fim, o sistema cruza os dados, apresenta uma prévia e permite salvar a planilha consolidada.</p></div>
    ...
    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">03</span><h3>Como funciona</h3><p>O fluxo reduz a complexidade técnica do cruzamento de dados e deixa as decisões de configuração visíveis para o usuário.</p></div><ol className="project-process"><li><span>01</span><h4>Selecionar</h4><p>O usuário escolhe as planilhas que deseja relacionar.</p></li><li><span>02</span><h4>Configurar</h4><p>Define a chave e as colunas de cada arquivo.</p></li><li><span>03</span><h4>Validar</h4><p>Confere a prévia e os registros sem correspondência.</p></li><li><span>04</span><h4>Exportar</h4><p>Salva o resultado em uma nova planilha Excel.</p></li></ol></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">04</span><h3>Conferência antes da exportação</h3></div><div className="project-security"><Icon name="shield" size={34}/><div><p>O ProcFácil não esconde as diferenças entre as fontes. Antes de salvar, a prévia destaca os registros sem correspondência e informa quantas linhas precisam de atenção. No arquivo final, essas ocorrências também são mantidas em uma aba específica.</p><div className="project-checks"><span><Icon name="check" size={18}/>Arquivos originais preservados</span><span><Icon name="check" size={18}/>Prévia do resultado consolidado</span><span><Icon name="check" size={18}/>Pendências separadas para revisão</span></div></div></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">05</span><h3>Resultado da demonstração</h3><p>Dados observados no teste apresentado com as planilhas de clientes, pagamentos e pedidos.</p></div><div className="project-metrics"><article><strong>3</strong><span>planilhas selecionadas</span><p>Três fontes relacionadas no mesmo fluxo.</p></article><article><strong>6</strong><span>registros consolidados</span><p>Chaves reunidas na prévia e no arquivo final.</p></article><article><strong>4</strong><span>linhas com pendências</span><p>Correspondências incompletas destacadas para revisão.</p></article></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">06</span><h3>Antes e depois</h3></div><div className="project-comparison"><article className="before"><header><div><strong>Antes</strong><p>Informações distribuídas em arquivos separados.</p></div><Icon name="warning"/></header><ul>{sources.map(file => <li key={file}><Icon name="file" size={16}/><span>{file}</span></li>)}</ul></article><article className="after"><header><div><strong>Depois</strong><p>Um arquivo estruturado para análise e conferência.</p></div><Icon name="check"/></header><ul>{output.map(([sheet, count]) => <li key={sheet}><Icon name="document" size={16}/><span>Aba {sheet}</span><small>{count}</small></li>)}</ul></article></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">07</span><h3>Tecnologias e competências</h3><p>Mais do que unir arquivos, o projeto exigiu pensar em regras de relacionamento, exceções e clareza para quem precisa conferir o resultado.</p></div><div className="project-skills-grid"><article><h4>Tecnologias e formatos</h4><div>{technologies.map(item => <span key={item}>{item}</span>)}</div></article><article><h4>Competências demonstradas</h4><div>{skills.map(item => <span key={item}>{item}</span>)}</div></article></div></section>
...
function InspectionCaseStudy() {
  ...
  return <>
    ...
    <section className="project-section project-challenge">
      <div><span className="project-section-number">01</span><h3>O contexto operacional</h3><p>A inspeção final é uma etapa importante para verificar se o produto atende aos critérios definidos antes da sua liberação. No contexto estudado, o inspetor precisa identificar o item, conferir diferentes pontos dimensionais e visuais, registrar o que foi encontrado e sinalizar possíveis não conformidades.</p><p>Quando esse processo depende de checklists em papel e registros distribuídos, o preenchimento pode variar, a localização do histórico exige mais esforço e a consolidação das informações de qualidade se torna menos prática.</p></div>
    ...
    <section className="project-section project-solution">
      <div className="project-section-copy"><span className="project-section-number">02</span><h3>A solução projetada</h3><p>Estruturei uma proposta de sistema que conduz o inspetor por etapas visíveis. A interface apresenta o item, o critério em análise, a referência visual e os limites de aceitação, permitindo registrar o valor encontrado e avançar somente com as informações necessárias.</p></div>
    ...
    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">03</span><h3>Como funciona</h3><p>O fluxo traduz a rotina de qualidade em uma sequência objetiva, mantendo cada decisão associada à inspeção correspondente.</p></div><ol className="project-process"><li><span>01</span><h4>Identificar</h4><p>O inspetor informa o item e os dados necessários para iniciar.</p></li><li><span>02</span><h4>Verificar</h4><p>Executa os critérios dimensionais e visuais apresentados.</p></li><li><span>03</span><h4>Registrar</h4><p>Inclui resultados, observações e possíveis não conformidades.</p></li><li><span>04</span><h4>Concluir</h4><p>Finaliza a inspeção e armazena o registro no histórico.</p></li></ol></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">04</span><h3>Padronização e rastreabilidade</h3></div><div className="project-security"><Icon name="shield" size={34}/><div><p>O sistema foi pensado para tornar as informações obrigatórias e os critérios visíveis durante a execução. Isso favorece registros mais consistentes, mantém o vínculo entre item, inspetor e resultado e facilita a recuperação do histórico.</p><div className="project-checks"><span><Icon name="check" size={18}/>Sequência de inspeção orientada</span><span><Icon name="check" size={18}/>Critérios e limites apresentados na tela</span><span><Icon name="check" size={18}/>Registro associado ao item inspecionado</span></div></div></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">05</span><h3>Informação organizada para a qualidade</h3><p>A proposta concentra os elementos essenciais da inspeção sem declarar resultados de produção ainda não medidos.</p></div><div className="project-metrics"><article><strong>01</strong><span>fluxo guiado</span><p>Etapas visíveis do início à conclusão.</p></article><article><strong>✓</strong><span>critérios estruturados</span><p>Referências e campos apresentados de forma consistente.</p></article><article><strong>↺</strong><span>histórico consultável</span><p>Registros organizados para futuras verificações.</p></article></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">06</span><h3>Antes e depois proposto</h3><p>Comparação conceitual entre a rotina baseada em registros dispersos e a estrutura digital projetada.</p></div><div className="project-comparison"><article className="before"><header><div><strong>Antes</strong><p>Processo apoiado em registros manuais e separados.</p></div><Icon name="warning"/></header><ul>{before.map(item => <li key={item}><Icon name="file" size={16}/><span>{item}</span></li>)}</ul></article><article className="after"><header><div><strong>Depois proposto</strong><p>Fluxo guiado e informações centralizadas.</p></div><Icon name="check"/></header><ul>{after.map(([step, detail]) => <li key={step}><Icon name="clipboard" size={16}/><span>{step}</span><small>{detail}</small></li>)}</ul></article></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">07</span><h3>Confidencialidade na apresentação</h3></div><div className="project-security"><Icon name="shield" size={34}/><div><p>As telas do portfólio utilizam exclusivamente dados fictícios. Códigos de peças, nomes, números de série, registros e especificações foram criados apenas para demonstrar a lógica da solução, sem expor informações de empresas ou produtos reais.</p><div className="project-checks"><span><Icon name="check" size={18}/>Identificadores fictícios</span><span><Icon name="check" size={18}/>Especificações demonstrativas</span><span><Icon name="check" size={18}/>Nenhum dado produtivo real</span></div></div></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">08</span><h3>Ferramentas e competências</h3><p>O projeto conecta conhecimento de processos industriais, qualidade, documentação e organização de dados.</p></div><div className="project-skills-grid"><article><h4>Ferramentas e métodos</h4><div>{technologies.map(item => <span key={item}>{item}</span>)}</div></article><article><h4>Competências demonstradas</h4><div>{skills.map(item => <span key={item}>{item}</span>)}</div></article></div></section>
...
function CalibrationCaseStudy() {
  ...
  return <>
    ...
    <section className="project-section project-solution">
      <div className="project-section-copy"><span className="project-section-number">02</span><h3>A solução projetada</h3><p>Estruturei uma proposta de sistema que reúne os instrumentos e seus dados de calibração em uma única interface. O painel organiza categorias, destaca vencimentos, apresenta a distribuição dos status e permite acessar cadastros, calendário, calibrações, relatórios e documentos a partir do mesmo fluxo.</p></div>
    ...
    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">03</span><h3>Como funciona</h3><p>O fluxo acompanha o instrumento desde o cadastro inicial até as próximas calibrações, preservando a continuidade dos registros.</p></div><ol className="project-process"><li><span>01</span><h4>Cadastrar</h4><p>Registra o instrumento e as informações necessárias ao controle.</p></li><li><span>02</span><h4>Acompanhar</h4><p>Calcula a validade e apresenta a proximidade do vencimento.</p></li><li><span>03</span><h4>Atualizar</h4><p>Registra nova calibração, certificado e situação do instrumento.</p></li><li><span>04</span><h4>Consultar</h4><p>Mantém o histórico disponível para buscas e conferências.</p></li></ol></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">04</span><h3>Prazos transformados em prioridades</h3></div><div className="project-security"><Icon name="shield" size={34}/><div><p>O painel foi pensado para não depender apenas da leitura de datas. A situação de cada instrumento é apresentada por status, permitindo diferenciar o que está no prazo, o que exige atenção e o que já está vencido.</p><div className="project-checks"><span><Icon name="check" size={18}/>Visão dos próximos vencimentos</span><span><Icon name="check" size={18}/>Status atualizado por instrumento</span><span><Icon name="check" size={18}/>Calendário para planejamento das calibrações</span></div></div></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">05</span><h3>Controle estruturado para a qualidade</h3><p>A apresentação demonstra capacidades da solução, sem atribuir indicadores de desempenho ainda não medidos.</p></div><div className="project-metrics"><article><strong>✓</strong><span>prazos visíveis</span><p>Datas e prioridades reunidas no mesmo painel.</p></article><article><strong>↺</strong><span>histórico preservado</span><p>Atualizações mantidas para futuras consultas.</p></article><article><strong>◎</strong><span>status centralizado</span><p>Situação dos instrumentos apresentada de forma consolidada.</p></article></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">06</span><h3>Antes e depois proposto</h3><p>Comparação conceitual entre o acompanhamento manual em planilha e a estrutura centralizada do sistema.</p></div><div className="project-comparison"><article className="before"><header><div><strong>Antes</strong><p>Controle distribuído e dependente de conferências manuais.</p></div><Icon name="warning"/></header><ul>{before.map(item => <li key={item}><Icon name="file" size={16}/><span>{item}</span></li>)}</ul></article><article className="after"><header><div><strong>Depois proposto</strong><p>Informações organizadas por função e prioridade.</p></div><Icon name="check"/></header><ul>{after.map(([area, detail]) => <li key={area}><Icon name="target" size={16}/><span>{area}</span><small>{detail}</small></li>)}</ul></article></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">07</span><h3>Exemplo de rastreabilidade</h3><p>Todos os identificadores abaixo são fictícios e servem somente para demonstrar a estrutura das informações.</p></div><div className="project-comparison"><article><header><div><strong>Instrumento</strong><p>Cadastro e localização</p></div><Icon name="target"/></header><ul><li><Icon name="clipboard" size={16}/><span>CAL-FIC-018</span><small>Paquímetro digital</small></li><li><Icon name="file" size={16}/><span>Patrimônio PAT-FIC-204</span><small>Laboratório A</small></li></ul></article><article className="after"><header><div><strong>Calibração</strong><p>Status e documentação</p></div><Icon name="check"/></header><ul><li><Icon name="document" size={16}/><span>Certificado CERT-FIC-087</span><small>Documento demonstrativo</small></li><li><Icon name="chart" size={16}/><span>A vencer</span><small>Prioridade sinalizada</small></li></ul></article></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">08</span><h3>Confidencialidade na apresentação</h3></div><div className="project-security"><Icon name="shield" size={34}/><div><p>Os códigos, patrimônios, certificados, fabricantes, datas, responsáveis e locais apresentados no portfólio são fictícios. A demonstração preserva integralmente dados de empresas e instrumentos reais.</p><div className="project-checks"><span><Icon name="check" size={18}/>Cadastros fictícios</span><span><Icon name="check" size={18}/>Datas e documentos demonstrativos</span><span><Icon name="check" size={18}/>Nenhuma informação empresarial real</span></div></div></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">09</span><h3>Ferramentas e competências</h3><p>O projeto conecta gestão de informações, controle documental, rastreabilidade e prevenção de falhas operacionais.</p></div><div className="project-skills-grid"><article><h4>Ferramentas e métodos</h4><div>{technologies.map(item => <span key={item}>{item}</span>)}</div></article><article><h4>Competências demonstradas</h4><div>{skills.map(item => <span key={item}>{item}</span>)}</div></article></div></section>
...
function DashboardCaseStudy() {
  ...
  return <>
    ...
    <section className="project-section project-challenge">
      <div><span className="project-section-number">01</span><h3>O desafio dos dados dispersos</h3><p>Muitas operações possuem os dados necessários, mas eles estão armazenados em planilhas criadas para registros do dia a dia, não para análise. Colunas, totais, observações e gráficos acabam misturados, enquanto diferentes versões do arquivo tornam a atualização e a comparação menos confiáveis.</p><p>O desafio do projeto foi transformar essa base em uma sequência compreensível: reconhecer o contexto dos dados, relacionar as colunas aos indicadores necessários e apresentar somente as informações úteis para o acompanhamento.</p></div>
    ...
    <section className="project-section project-solution">
      <div className="project-section-copy"><span className="project-section-number">02</span><h3>A solução desenvolvida</h3><p>Desenvolvi uma aplicação web que conduz o usuário desde o envio da planilha até a geração do dashboard. A ferramenta analisa os nomes das colunas, sugere a categoria mais compatível, apresenta o mapeamento para conferência e utiliza os campos confirmados para estruturar indicadores e visualizações.</p></div>
    ...
    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">03</span><h3>Como funciona</h3><p>O fluxo torna visível o caminho entre a fonte de dados e o resultado apresentado no painel.</p></div><ol className="project-process"><li><span>01</span><h4>Enviar</h4><p>O usuário seleciona a planilha que contém os dados.</p></li><li><span>02</span><h4>Classificar</h4><p>A aplicação sugere a categoria com base nas colunas encontradas.</p></li><li><span>03</span><h4>Mapear</h4><p>O usuário revisa e confirma a relação entre os campos.</p></li><li><span>04</span><h4>Analisar</h4><p>O painel apresenta indicadores, gráficos, filtros e dados detalhados.</p></li></ol></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">04</span><h3>Validação antes da visualização</h3></div><div className="project-security"><Icon name="shield" size={34}/><div><p>O projeto não transforma qualquer coluna em gráfico automaticamente. Antes de gerar o dashboard, o usuário confere os campos obrigatórios e pode ajustar o mapeamento sugerido. Essa etapa mantém as regras da análise visíveis e reduz o risco de interpretar uma coluna incorreta.</p><div className="project-checks"><span><Icon name="check" size={18}/>Campos obrigatórios identificados</span><span><Icon name="check" size={18}/>Sugestões revisáveis pelo usuário</span><span><Icon name="check" size={18}/>Mapeamento confirmado antes do painel</span></div></div></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">05</span><h3>Informação organizada para análise</h3><p>O dashboard financeiro demonstrativo reúne três perspectivas complementares, sem atribuir ganhos operacionais ainda não medidos.</p></div><div className="project-metrics"><article><strong>◎</strong><span>indicadores principais</span><p>Receita, despesa e saldo apresentados em destaque.</p></article><article><strong>↗</strong><span>evolução temporal</span><p>Comparação dos resultados ao longo dos períodos.</p></article><article><strong>≡</strong><span>composição dos dados</span><p>Distribuição por centro de custo e tipo de conta.</p></article></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">06</span><h3>Recursos para investigação</h3></div><div className="project-feature-grid"><FeatureCard icon="search" title="Filtros por período">Permitem analisar janelas de tempo predefinidas ou personalizadas.</FeatureCard><FeatureCard icon="target" title="Filtros por categoria">Refinam a leitura por centro de custo e tipo de conta.</FeatureCard><FeatureCard icon="eye" title="Dados detalhados">Mantêm as linhas que deram origem aos indicadores disponíveis para conferência.</FeatureCard><FeatureCard icon="report" title="Saídas para compartilhamento">A interface oferece opções de relatório e exportação dos dados analisados.</FeatureCard></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">07</span><h3>Antes e depois</h3><p>Comparação entre uma base criada para registro manual e uma visão preparada para acompanhamento operacional.</p></div><div className="project-comparison"><article className="before"><header><div><strong>Antes</strong><p>Dados misturados e análise dependente de trabalho manual.</p></div><Icon name="warning"/></header><ul>{before.map(item => <li key={item}><Icon name="file" size={16}/><span>{item}</span></li>)}</ul></article><article className="after"><header><div><strong>Depois</strong><p>Fluxo estruturado da importação até a análise.</p></div><Icon name="check"/></header><ul>{after.map(([stage, detail]) => <li key={stage}><Icon name="chart" size={16}/><span>{stage}</span><small>{detail}</small></li>)}</ul></article></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">08</span><h3>Dados protegidos na apresentação</h3></div><div className="project-security"><Icon name="shield" size={34}/><div><p>Os valores, períodos, centros de custo, categorias e registros apresentados no portfólio são fictícios ou demonstrativos. Nenhuma informação confidencial de empresas reais é utilizada para explicar o funcionamento da solução.</p><div className="project-checks"><span><Icon name="check" size={18}/>Valores demonstrativos</span><span><Icon name="check" size={18}/>Categorias genéricas</span><span><Icon name="check" size={18}/>Nenhuma base empresarial real</span></div></div></div></section>

    <section className="project-section"><div className="project-section-copy"><span className="project-section-number">09</span><h3>Tecnologias e competências</h3><p>O projeto combina tratamento de dados, definição de regras e comunicação visual voltada para o acompanhamento da operação.</p></div><div className="project-skills-grid"><article><h4>Tecnologias e recursos</h4><div>{technologies.map(item => <span key={item}>{item}</span>)}</div></article><article><h4>Competências demonstradas</h4><div>{skills.map(item => <span key={item}>{item}</span>)}</div></article></div></section>
